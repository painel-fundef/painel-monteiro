...
function tipoAcao(rows){
  ...
  c.innerHTML=d.map(([k,v])=>`<div class="bar ${filters.tipo===k?'active':''}" data-fk="tipo" data-fv="${k.replace(/"/g,'&quot;')}" style="grid-template-columns:140px 1fr 36px"><span class="bl" title="${k}">${k}</span><span class="track"><span class="fill" style="width:${v/max*100}%"></span></span><span class="bv">${v}</span></div>`).join('');
...
function situacao(rows){
  ...
  c.innerHTML=`
    <div class="sit-card fav ${filters.situacao==='Favorável'?'active':''}" data-sit="Favorável"><div class="sit-icon">✓</div><div class="sit-body"><div class="sit-label">Favorável</div><div class="sit-num">${fav.length}</div><div class="sit-pct">${pct(fav.length)} da carteira</div></div></div>
    <div class="sit-card desf ${filters.situacao==='Desfavorável'?'active':''}" data-sit="Desfavorável"><div class="sit-icon">✕</div><div class="sit-body"><div class="sit-label">Desfavorável</div><div class="sit-num">${desf.length}</div><div class="sit-pct">${pct(desf.length)} da carteira</div></div></div>
  ...
  c.querySelectorAll('.sit-card').forEach(card=>card.onclick=()=>toggleFilter('situacao',card.dataset.sit));
}
...
function rankList(cid,tid,rows,key,fk){
  ...
  c.innerHTML=agg.slice(0,12).map(([k,o],i)=>{const val=o.val>0?BRLk(o.val):(o.valc>0?BRLk(o.valc)+' cont.':'—');
    return `<div class="rkrow ${filters[fk]===k?'active':''}" data-fk="${fk}" data-fv="${k.replace(/"/g,'&quot;')}"><span class="rkidx">${i+1}</span><span><span class="rkn" title="${k}">${k}</span></span><span class="rkmeta"><span class="rkn-proc">${o.nproc}</span> <span class="rksub">proc.</span><br><span class="rkn-val">${val}</span></span></div>`;}).join('');
  ...
  c.innerHTML=Object.entries(tribs).sort((a,b)=>b[1].n-a[1].n).map(([t,o])=>{
    ...
    const varasHTML=varas.map(([vk,vo])=>{
      const leaves=[...vo.procs].sort((a,b)=>b.valEnv-a.valEnv).map(r=>{
        ...
        return `<div class="proc-leaf" data-p="${r.proc}"><div><div class="pln">${r.parte}</div><div class="plm">${r.grau.replace(' GRAU','°')} · ${mag} · ${r.materia||r.tipoAcao||'—'}</div></div><div class="plv">${r.valEnv>0?BRLk(r.valEnv):'—'}</div></div>`;}).join('');
...
function multiInstance(rows){
  ...
  c.innerHTML=multi.slice(0,14).map(([k,v,ts])=>{const val=v.reduce((s,r)=>s+r.valEnv,0);
    return `<div class="rkrow" data-p="${k.replace(/"/g,'&quot;')}"><span class="rkidx">${[...ts].length}</span><span><span class="rkn" title="${k}">${k}</span><div class="rksub">${[...ts].sort().join(' · ')}</div></span><span class="rkmeta"><span class="rkn-proc">${v.length}</span> <span class="rksub">proc.</span>${val>0?'<br><span class="rkn-val">'+BRLk(val)+'</span>':''}</span></div>`;}).join('');
...
function pendChips(rows){
  ...
  c.innerHTML=d.map(([k,v])=>`<div class="pend-chip ${filters.pendencia===k?'active':''}" data-pv="${k.replace(/"/g,'&quot;')}">
    <div class="pc-label">${k}</div><div class="pc-num">${v}</div></div>`).join('');
  c.querySelectorAll('.pend-chip').forEach(ch=>ch.onclick=()=>toggleFilter('pendencia',ch.dataset.pv));
}
function timeline(rows){
  ...
  c.innerHTML=pend.map(r=>`
    <div class="pt-row" data-p="${r.proc}">
  ...
  c.querySelectorAll('.pt-row').forEach(x=>x.onclick=()=>focusProc(x.dataset.p));
}
